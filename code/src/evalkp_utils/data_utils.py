# code adapted from https://github.com/google/lassie/blob/main/utils/data_utils.py
...
def crop_and_resize(img, bb, crop_size, rgb=True, norm=True, gpu=True):
    if rgb:
        img = torch.from_numpy(img).permute(2,0,1)[None,:,:,:].float()
...
def find_nearest_vertex(keypoints, verts_2d, verts_vis):
    ''' keypoints: N * 2; verts_2d: N * 2; verts_vis: N'''
