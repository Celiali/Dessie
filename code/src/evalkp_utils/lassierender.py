# code adapted from https://github.com/google/lassie/blob/main/utils/rendering.py
...
from pytorch3d.renderer import (
    ...
    SoftPhongShader,
    HardPhongShader,
    Textures,OpenGLPerspectiveCameras
)
import sys
...
class Renderer():
    def __init__(self, device, mode='text'):
        ...
        fov = 2 * np.arctan(cfg.input_size[0] / (focal * 2)) * 180 / np.pi
        self.camera = OpenGLPerspectiveCameras(zfar=350, fov=fov, R = self.R, T = self.T, device=self.device)       
        ...
        if mode == 'sil':
            ...
            renderer = MeshRenderer(
                rasterizer=MeshRasterizer(cameras=self.camera, raster_settings=raster_settings),
                shader=SoftPhongShader(device=device, cameras=self.camera, lights=self.light, blend_params=blend_params)
            )
            ...
            renderer = MeshRenderer(
                rasterizer=MeshRasterizer(cameras=self.camera, raster_settings=raster_settings),
                shader=HardPhongShader(device=device, cameras=self.camera, lights=self.light)
            )
    ...
    def project(self, x):
        return self.camera.transform_points_screen(x, image_size=cfg.input_size)
    ...
    def get_part_masks(self, verts, faces):
        bs= verts.shape[0]
        if len(verts.shape) == 3:
            verts_combined = verts #[1,v,3]
            faces_combined = faces #[1,F,3]
    ...
    def get_verts_vis(self, verts, faces):
        ...
        return visibility_map #torch.stack(verts_vis, 0)
