...
from src.utils.CONSTANCT import TEXTURE_Neworder
import gc
...
class DessiePIPE(Dataset):
    def __init__(self, args, device, length, FLAG):
        print(f'dataset: DessiePIPE: {FLAG}')
        self.length = length
...
if __name__ == '__main__':
    ...
    print(args)
    dataset = DessiePIPE(args=args, device=device, length=1000, FLAG='TRAIN')

    train_dl = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, num_workers=0, shuffle=False, pin_memory=True)    
    VISUAL = True
    save_path = '/home/x_cili/x_cili_lic/DESSIE/results/dataloader'
    start = time.time()
    for i, batch in enumerate(train_dl):
        ...
        print(label_tensor.shape) #torch.Size([4, 1])
        ...
        print(label_tensor.shape)  # [batch_size, 1, data_batch_size] --> modified to [batch_size, data_batch_size]
        ...
        print(kp_3d_tensor.shape) #[batch, data_batch_size, 1,3]
        print(kp2d_tensor.shape) #[batch, data_batch_size, 1,3]
        # init_images_tensor: torch.Size([32, 2, 3, 256, 256]) 
        # mask_image_tensor: torch.Size([32, 2, 1, 256, 256])
        # kp_2d_tensor: torch.Size([32, 2, 17, 3])
        # label_tensor:  torch.Size([32, 1]) 
            ...
            for t in range(batch_size):
                ...
                for tt, keypoint in enumerate(kp2d[t]):
                    ...
                    if flag == 1:
                        ...
                        # drawing text size
                if args.getPairs:
                    draw.text((5, 5), text=f"label:{label[int(t / 2), 0]}", fill=(255,0,0),
                              font=ImageFont.load_default(), align="left")
