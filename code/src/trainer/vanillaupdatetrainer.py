...
class VanillaUpdateTrainer(pl.LightningModule):
    def __init__(self, opts):
        super().__init__()
        print("trainer: update version trainer")
        # set trainable model
    ...
    @torch.no_grad()
    def predict(self, input_image, cameraindexclass_tensor=None, pose_encoded_text = None):
        self.initial_setup()
        self.model.eval()
        if self.opts.ModelName in self.HMRKEYS:
            pred_rotmat, pred_betas, pred_cam_crop = self.model(input_image)
        ...
        pred_vertices_crop, _, _ = self.get_SMAL_results(pred_betas, pred_rotmat, pred_trans_cam_crop)
        pred_kp3d_crop = get_point(pred_vertices_crop)
        predseg_crop, predp2d_crop = self.get_render_results(verts=pred_vertices_crop, faces=self.faces,
                                                             points=pred_kp3d_crop)

        color_image = self.get_color_results(verts=pred_vertices_crop, faces=self.faces)
        return {'pred_betas': pred_betas, 'pred_rotmat': pred_rotmat, 'pred_trans_cam_crop': pred_trans_cam_crop,
                'pred_kp2d_crop': predp2d_crop, 'pred_mask_crop': predseg_crop, 'pred_color_img': color_image, 'pred_cam_crop': pred_cam_crop,
                'pred_vertices_crop': pred_vertices_crop}

    @torch.no_grad()
    def easypredict(self, xf_shape=None, xf_pose=None, xf_cam=None, xf=None, cameraindexclass_tensor = None, pose_encoded_text = None):
        self.model.eval()
        self.initial_setup()
        # convert the camera parameters from the crop camera
        if self.opts.ModelName in self.DISENKEYS:
            pred_rotmat, pred_betas, pred_cam_crop = self.model.predict_from_latent(xf_shape, xf_pose, xf_cam)
        elif self.opts.ModelName in self.HMRKEYS:
            batch_size = xf.shape[0]
            pred_rotmat, pred_betas, pred_cam_crop = self.model.predict(xf, init_pose=self.model.init_pose.expand(batch_size, -1),
                                                                        init_shape=self.model.init_shape.expand(batch_size, -1),
                                                                        init_cam=self.model.init_cam.expand(batch_size,-1))
        else:
            raise ValueError
        pred_trans_cam_crop = self.cam_bbox(pred_cam_crop, r=self.img_size[0], focal_length=self.FOCAL_LENGTH_BBOX)
        pred_vertices_crop, _, _ = self.get_SMAL_results(pred_betas, pred_rotmat, pred_trans_cam_crop)
    ...
    @torch.no_grad()
    def latentpredict(self, input_image):
        self.model.eval()
        self.initial_setup()
        # convert the camera parameters from the crop camera
        if self.opts.ModelName in self.HMRKEYS:
            xf = self.model.obtain_latent_space(input_image)
            return {'xf': xf}
        elif self.opts.ModelName in self.DISENKEYS:
            xf_shape, xf_pose, xf_cam = self.model.obtain_latent_space(input_image)
            return {'xf_shape': xf_shape, 'xf_pose': xf_pose, 'xf_cam': xf_cam}
        else:
            raise ValueError
