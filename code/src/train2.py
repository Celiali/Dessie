...
from src.dataset2.basedataset7 import DessiePIPE
import os, random
import numpy as np
...
def get_syn_dataset(args, device, train_length=1000, valid_length=30):
    if args.DatasetName == 'DessiePIPE':
        dataset = DessiePIPE(args=args, device=device, length=train_length, FLAG='TRAIN')
        validdataset = DessiePIPE(args=args, device=device, length=valid_length, FLAG='VALID')
    else:
